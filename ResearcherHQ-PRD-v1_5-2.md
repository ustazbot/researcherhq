...
**Nota:** Tambahan §6C (User Account & Support) — completion of existing scope, bukan perubahan struktur. Billing gateway ditukar BillPlz → ToyyibPay (komponen swap dalam abstraksi sama, lihat nota §4). Version kekal 1.5; v1.6 reserved untuk perubahan luar jangka.
...
| Billing | ToyyibPay (FPX/DuitNow) | Locked* |
| Export | python-docx (.docx, async queue) | Locked |
| LaTeX | KaTeX (frontend) | Locked |
| Reverse proxy | Nginx | Locked |

*\*ToyyibPay menggantikan BillPlz — boleh daftar tanpa SSM (laluan personal/DEGM), fee RM1 flat/transaksi, settlement next business day. **BELUM VERIFIED:** sokongan auto-recurring subscription (model langganan bulanan RM39). Kena sahkan dengan support ToyyibPay sebelum build Fasa 2 — jika tiada auto-deduction, billing-cycle logic kena handle dalam FastAPI (lihat §14 Fasa 2). Migrasi personal → company account (selepas SSM) = swap `.env` API key sahaja, code tak berubah.*
...
├── Manage Subscription
│   ├── Pro: cancel/downgrade, billing history (ToyyibPay)
│   └── Free: upgrade CTA
├── Account Settings
...
```
├── ToyyibPay integration, topup, kredit display (visual tank)
│   └── PREREQ: sahkan auto-recurring support dulu (lihat nota §4).
│       Jika tiada → build billing-cycle reminder/renewal logic sendiri.
...
| ToyyibPay webhook | 100% → kredit <30s | 5 transaksi sebenar (kecil) |
| Recurring/renewal flow | Langganan bulanan deduct/renew tepat | Test 1 cycle penuh (atau manual reminder jika tiada auto-recurring) |
...
| ToyyibPay auto-recurring support | **Perlu verify dengan support sebelum Fasa 2** — affect scope billing |
| Daftar SSM + akaun bank syarikat | Dalam proses — selepas siap, upgrade ToyyibPay personal → company account |
| Keselesaan syariah mekanisme DEGM (ToyyibPay personal) | Bos rujuk pihak berautoriti — di luar skop teknikal |
